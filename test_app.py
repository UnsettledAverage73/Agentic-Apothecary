...
process = subprocess.Popen(
    ["uv", "run", "python", "main.py"], stdout=subprocess.PIPE, stderr=subprocess.PIPE
)
...
try:
    ...
    order = {"patient_id": "PAT004", "product_name": "Mucosolvan", "quantity": 1}
    response = requests.post("http://localhost:8000/agent/order", json=order)
    print("Test Rx Required Order Response:", response.json())

    # Test Not in Formulary
    order = {"patient_id": "PAT001", "product_name": "Unknown Medicine", "quantity": 1}
    response = requests.post("http://localhost:8000/agent/order", json=order)
